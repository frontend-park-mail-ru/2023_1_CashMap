...
class messagesStore {
     ...
     * @param {Date} lastMessageDate - дата, после которой выбираются сообщения
     */
    async _getChatsMsg(chatId, count, lastMessageDate, isScroll=false) {
        ...
        if (request.status === 200) {
            ...
            if (!response.body.messages || response.body.messages.length === 0) {
                ...
                response.body.messages.forEach((message) => {
                    ...
                    message.raw_creation_date = message.creation_date.replace("+", "%2B");
