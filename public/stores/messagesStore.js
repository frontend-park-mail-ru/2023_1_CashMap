            ...
            this.chats.forEach((chat) => {
                if (chat.members.length === 1) {
                    if (!chat.members[0].avatar_url) {
                        ...
                        chat.members[0].avatar_url = `https://${Ajax.backendHostname}/${ chat.members[0].avatar_url }`;
                    }
                    chat.avatar_url = `https://${Ajax.backendHostname}/${ chat.members[0].avatar_url }`;
                    ...
                    chat.members.forEach((member) => {
                        if (!member.avatar_url) {
                            ...
                            member.avatar_url = `https://${Ajax.backendHostname}/${ member.avatar_url }`;
                        }
        ...
        if (request.status === 200) {
            ...
            this.messages.forEach((message) => {
                if (!message.sender_info.avatar_url) {
                    ...
                    message.sender_info.avatar_url = `https://${Ajax.backendHostname}/${ message.sender_info.avatar_url }`;
                }
